...
import { Ticket, ArrowLeft, Plus, Trash2, Send, Layout, MousePointer2, Type, Palette, Hash, UserPlus, MessageCircle, ShieldAlert, Check, ChevronDown, BellRing, Image as ImageIcon, AlignLeft, Search, X, Loader2, User, Eye, EyeOff, Megaphone } from 'lucide-react';
...
export default function NewTicketPanelPage() {
  ...
  const [title, setTitle] = useState('Skontaktuj się z administracją');
  const [description, setDescription] = useState('Kliknij przycisk poniżej, aby otworzyć ticket i otrzymać pomoc.');
  ...
  const [buttons, setButtons] = useState<ButtonConfig[]>([
    {
      ...
      staffRoleIds: [],
      addRoleIds: [],
      message: 'Witaj! Zaraz ktoś Ci pomoże. Opisz swój problem poniżej.',
      ticketTitle: 'Centrum Pomocy',
      ticketFooter: 'Nasz zespół odpowie tak szybko, jak to możliwe.',
      ticketColor: '#6366f1',
  ...
  const loadData = async () => {
    try {
      ...
      if (allChannels.length > 0) setChannelId(allChannels.find((c: any) => c.type === 0)?.id || allChannels[0].id);
    } catch (err) {
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-7xl mx-auto space-y-8 pb-12 px-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Kreator Systemu Ticketów</h1>
            <p className="text-foreground-secondary text-sm">Zbuduj profesjonalną ścieżkę pomocy dla swoich użytkowników.</p>
          </div>
        </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="space-y-8">
                   ...
                   {/* Action Details with Prettier Selectors */}
                   <div className="grid grid-cols-1 gap-6">
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              <div className="space-y-4">
                                ...
                                {currentBtn.showStaffButton && (
                                   <div className="space-y-2 p-4 bg-background-tertiary/50 rounded-2xl border border-border/50">
                                      ...
                                      />
                                      <p className="text-[9px] text-foreground-subtle italic">Zostanie wysłana wraz z pingiem ról wsparcia.</p>
                                   </div>
                                )}
                              </div>
                              ...
                              <div className="space-y-4 bg-background-tertiary/50 p-6 rounded-2xl border border-border/50">
                                 ...
                                 <div className="space-y-4">
                                    ...
                                    <input type="text" value={currentBtn.ticketFooter} onChange={(e) => updateButton(currentBtn.id, { ticketFooter: e.target.value })} placeholder="Stopka (zostaw puste aby usunąć)..." className="w-full bg-background-tertiary border border-border rounded-xl px-4 py-2.5 text-xs outline-none focus:border-blue-400" />
                      ...
                      {currentBtn.actions.includes('SEND_MESSAGE') && !currentBtn.actions.includes('OPEN_TICKET') && (
                         <div className="p-6 bg-amber-500/5 rounded-3xl border border-amber-500/10 space-y-2">
                            <label className="text-[10px] font-bold text-amber-400 uppercase">Wiadomość Prywatna (Widoczna tylko dla klikającego)</label>
                            <textarea value={currentBtn.message} onChange={(e) => updateButton(currentBtn.id, { message: e.target.value })} className="w-full bg-background-tertiary border border-border rounded-xl px-4 py-3 text-sm outline-none resize-none h-24" placeholder="Podaj treść komunikatu..." />
                         </div>
                      )}
                   </div>
                </div>
          ...
          <div className="sticky top-24 space-y-6">
              ...
              <div className="p-5 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className="text-[11px] text-[#949ba4]">Dzisiaj o 02:22</span>
                  </div>
                  ...
                  {previewMode === 'PANEL' ? (
                    ...
                    <div className="space-y-4">
                       ...
                       <div className="border-l-[4px] rounded-[4px] bg-[#2b2d31] p-3.5" style={{ borderColor: currentBtn.ticketColor || '#6366f1' }}>
                          <div className="text-white font-bold text-[16px] mb-1.5">
                            {currentBtn.ticketTitle || `Ticket: ${currentBtn.label}`}
                          </div>
                          <div className="text-[#dbdee1] text-[14px] whitespace-pre-wrap leading-relaxed">
                            {currentBtn.message || 'Witaj! Zaraz ktoś Ci pomoże. Opisz swój problem poniżej.'}
                          </div>
                          {currentBtn.ticketFooter && (
                            <div className="mt-3 text-[11px] text-[#949ba4] border-t border-[#3f4147] pt-2 flex items-center gap-2">
                               {currentBtn.ticketFooter}
                            </div>
                          )}
                       </div>
                       <div className="flex flex-wrap gap-2.5">
                          {currentBtn.showCloseButton && (
                            <div className="px-3.5 py-2 rounded-[3px] bg-[#da373c] text-white text-[13px] font-medium flex items-center gap-2 shadow-sm">
                              🔒 {currentBtn.closeButtonLabel || 'Zamknij Ticket'}
                            </div>
                          )}
                          {currentBtn.showStaffButton && (
                            <div className="px-3.5 py-2 rounded-[3px] bg-[#4e5058] text-white text-[13px] font-medium flex items-center gap-2 shadow-sm">
                              🔔 {currentBtn.staffButtonLabel || 'Wezwij Administrację'}
                            </div>
                          )}
                       </div>
                    </div>
            ...
            <div className="glass-card p-6 border-l-4 border-l-amber-500">
               <p className="text-xs text-foreground-secondary leading-relaxed">
                 Możesz teraz ustawić dedykowaną wiadomość dla administracji, np.: <b>"Uwaga, potrzebna pomoc przy płatnościach!"</b>. Zostanie ona wysłana po kliknięciu przycisku {currentBtn.staffButtonLabel}.
               </p>
            </div>
          </div>
        </div>
