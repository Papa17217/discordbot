...
import { Terminal, Shield, Trash2, Loader2, Circle, AlertCircle, Info, Settings, RefreshCcw, Layers } from 'lucide-react';
  ...
  if (user?.role !== 'OWNER' && user?.role !== 'SUPER_ADMIN') {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        ...
        <p className="text-foreground-secondary">Tylko główni administratorzy mogą przeglądać konsolę systemową.</p>
      </div>
    );
  }
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="glass-card border-accent/20 overflow-hidden flex flex-col shadow-2xl shadow-accent/5">
        ...
        <div className="p-3 bg-background-elevated/50 border-t border-border flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 opacity-50">
             <kbd className="px-1.5 py-0.5 rounded bg-white/5 border border-border text-[10px]">ENTER</kbd>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="glass-card p-4 flex items-center gap-4 border-emerald-500/20">
          ...
          <div>
            <p className="text-xs text-foreground-secondary">Poziom logowania</p>
            <p className="font-bold">INFO / DEBUG</p>
          </div>
        </div>
        <div className="glass-card p-4 flex items-center gap-4 border-amber-500/20">
          ...
          <div>
            <p className="text-xs text-foreground-secondary">Bufor logów</p>
            <p className="font-bold">{logs.length} / 500</p>
          </div>
        </div>
        <div className="glass-card p-4 flex items-center gap-4 border-accent/20">
          <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center">
            <Settings className="w-5 h-5 text-accent" />
          </div>
          <div>
            <p className="text-xs text-foreground-secondary">Silnik bota</p>
            <p className="font-bold">discord.js v14</p>
          </div>
        </div>
      </div>
    </motion.div>
