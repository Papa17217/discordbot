// ============================================
// API Bridge — Internal HTTP API for Public Bot ↔ API
// ============================================
...
export function startBridge(client: BotClient) {
  ...
  const SECRET = process.env.BOT_PUBLIC_API_SECRET || 'internal-bot-public-api-secret';
  ...
  // ── Start serwera ────────────────────────
  const port = process.env.BOT_PUBLIC_API_PORT || 4002;
  app.listen(port, () => {
    logger.info(`🌉 Public Bot API Bridge działa na porcie ${port}`);
  });
}
